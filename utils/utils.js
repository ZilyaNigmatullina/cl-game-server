...
function playerPositionGenerator(fieldSize) {
    const move1 = !!Math.round(Math.random())
    const move2 = !move1

    const position1 = [move1 ? 0 : (fieldSize - 1), randomInt(0, fieldSize)]
    const position2 = [!move1 ? 0 : (fieldSize - 1), randomInt(0, fieldSize)]
...
function barrierPositionGenerator(fieldSize, barrierCount) {
    const barriers = []
    for (let i = 0; i < barrierCount; i++) {
        const position = [randomInt(0, fieldSize), randomInt(0, fieldSize)]
        const dx = Math.round(Math.random()) === 1 ? 1 : -1
        const dy = Math.round(Math.random()) === 1 ? 1 : -1
        const type = Math.round(Math.random())
        console.log(dx, dy, type)
        barriers.push({
            position: [
                position,
